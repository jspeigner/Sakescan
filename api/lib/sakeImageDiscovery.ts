...
export async function searchSakeImageCandidates(
...
): Promise<{ images: SearchImageRow[]; sakeData?: SakeSearchMeta }> {
  const { name, nameJapanese, brewery } = params;
  const searchQuery = buildSakeImageSearchQuery(name, nameJapanese ?? undefined, brewery ?? undefined);
  ...
  const googleRows = await scrapeGoogleImages(firecrawlApiKey, searchQuery);
  results.push(...googleRows);
