...
describe('cron auth', () => {
  ...
  test('recognizes Vercel cron header', () => {
    ...
    expect(isVercelCronRequest({ 'x-vercel-cron-schedule': '*/15 * * * *' })).toBe(false);
  ...
  test('still accepts current scheduler metadata when CRON_SECRET is configured', () => {
    const currentScheduler = {
      'user-agent': 'vercel-cron/1.0',
      'x-vercel-cron-schedule': '*/10 * * * *',
    };
    expect(isAuthorizedCronRequest(currentScheduler, 'secret-1')).toBe(true);
    expect(isAuthorizedCronRequest({ 'x-vercel-cron': '1' }, 'secret-1')).toBe(true);
  });
});
