...
export async function runWineEngineSyncBatch(
  ...
  const initialBatch = await query.range(offset, offset + batchSize - 1);
  if (initialBatch.error) throw new Error(initialBatch.error.message);
  let rows = initialBatch.data;
