  ...
  try {
    ...
    if (firecrawlKey && openaiKey && !rateLimited && !hitTimeBudget) {
      ...
      for (let page = 0; page < DISCOVER_POOL_MAX_PAGES; page++) {
        ...
        const from = page * DISCOVER_POOL_LIMIT;
        ...
        if (missingPoolError) {
          const msg = missingPoolError.message.slice(0, 140);
          diagnostics.discover.perRowErrors++;
          pushSample(diagnostics.discover.rowErrorSamples, `missing pool page ${page + 1}: ${msg}`);
          errors.push(`discover pool: ${msg}`);
          break;
        }

        const poolRows = ((missingPool || []) as SakeRow[]);
        diagnostics.discover.poolRows += poolRows.length;
        diagnostics.discover.poolPagesScanned++;

        const missingRows = poolRows.filter((row) => {
        ...
        if (dueRows.length >= discoverRowCapThisRun || poolRows.length < DISCOVER_POOL_LIMIT) break;
      }
    ...
    const heartbeat = {
      ...
      diagnostics: {
        ...
        discoverPoolPagesScanned: diagnostics.discover.poolPagesScanned,
